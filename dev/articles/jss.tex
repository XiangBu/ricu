...
R+   Surv(charttime, next_charttime, event) ~ lact + sofa,
R+   data = tbl
R+ )
\end{CodeInput}
\end{CodeChunk}
...
\begin{center}\includegraphics{/tmp/RtmpVwWQkB/file39665d73f79c/dev/articles/jss_files/figure-latex/cox_plot-1} \end{center}
...
\begin{CodeOutput}
removed 1625 (50.4%) of rows due to out of range entries
\end{CodeOutput}

\begin{CodeOutput}
multiple units detected: Uhr (90.49%), units/hr (9.51%)
\end{CodeOutput}
...
R+   list(ins_sum = .bincode(sum(ins), breaks = c(-Inf, ins_breaks, Inf)) - 1),
...
\begin{center}\includegraphics{/tmp/RtmpVwWQkB/file39665d73f79c/dev/articles/jss_files/figure-latex/diabetes_visualize-1} \end{center}
